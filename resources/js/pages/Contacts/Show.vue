    ...
    <div class="flex flex-col min-h-full bg-app-bg text-app" style="font-family: Inter, sans-serif;">
        ...
        <div class="flex items-center justify-between gap-4 px-6 py-4 border-b border-app">
            <div class="flex items-center gap-3">
                <Link href="/contacts" class="flex items-center gap-1.5 text-xs font-semibold text-slate-400 hover:text-app border border-app hover:border-slate-600 px-3 py-2 rounded-lg transition-colors">
            ...
            <div class="flex items-center gap-2 shrink-0">
                <Link :href="`/contacts/${contact.data.id}/edit`" class="flex items-center gap-1.5 text-xs font-semibold text-slate-400 border border-app hover:text-app hover:border-slate-600 px-3 py-2 rounded-lg transition-colors">
        ...
        <div class="px-6 py-6 grid gap-4 lg:grid-cols-3">
            ...
            <div class="rounded-xl border border-app bg-app-surface overflow-hidden">
                ...
                <div class="px-5 py-4 flex flex-col gap-3">
                    ...
                    <div class="flex items-center gap-2.5 text-sm">
                        ...
                        <span v-else class="text-slate-600">No organization</span>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 rounded-xl border border-app bg-app-surface overflow-hidden">
                <div class="px-5 py-4 border-b border-app flex items-center justify-between gap-3">
                    ...
                    <span class="text-xs font-semibold px-2 py-0.5 rounded-full border border-app text-slate-400 bg-app-elevated">{{ deals.data.length }}</span>
                </div>
                <div class="divide-y divide-app">
                    <div v-for="deal in deals.data" :key="deal.id" class="px-5 py-3 flex items-center justify-between gap-3">
                        <div class="min-w-0">
                            ...
                            <p class="text-xs text-slate-500 mt-0.5">{{ deal.stage?.name ?? 'No stage' }}</p>
                        </div>
                        <p v-if="deal.value != null" class="text-sm font-bold text-app shrink-0">${{ Number(deal.value).toLocaleString() }}</p>
                        <p v-else class="text-xs text-slate-600 shrink-0">No value</p>
                    </div>
                    <div v-if="deals.data.length === 0" class="px-5 py-10 text-center">
                        <TrendingUp class="mx-auto mb-2 size-6 text-slate-600" />
                        <p class="text-sm text-slate-500">No deals linked yet.</p>
                    </div>
                </div>
            </div>
        </div>
