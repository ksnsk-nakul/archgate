...
<template>
    ...
    <section class="py-20 px-6 text-center" style="background: linear-gradient(135deg, #051424 0%, #0a1929 60%, #051424 100%);">
        <div class="max-w-3xl mx-auto">
            <span class="inline-flex items-center gap-2 bg-[#f7600d]/10 border border-[#f7600d]/20 text-[#f7600d] text-xs font-semibold uppercase tracking-widest px-3 py-1 rounded-full mb-6">Contact</span>
        ...
        <div class="max-w-5xl mx-auto grid md:grid-cols-2 gap-12">
            ...
            <div class="flex flex-col gap-6">
                ...
                <div v-if="contactEmail" class="flex items-start gap-4">
                    <div class="size-10 rounded-xl bg-[#f7600d]/10 flex items-center justify-center shrink-0">
                        <svg class="size-5 text-[#f7600d]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                    </div>
                    <div>
                        <p class="text-xs text-slate-500 font-semibold uppercase tracking-wider mb-1">Email</p>
                        <a :href="`mailto:${contactEmail}`" class="text-sm text-slate-300 hover:text-[#f7600d] transition-colors">{{ contactEmail }}</a>
                    </div>
                </div>
                <div v-if="contactPhone" class="flex items-start gap-4">
                    <div class="size-10 rounded-xl bg-[#f7600d]/10 flex items-center justify-center shrink-0">
                        <svg class="size-5 text-[#f7600d]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                    </div>
                    <div>
            ...
            <div class="rounded-2xl border border-slate-800 bg-[#0d1c2d] p-6">
                ...
                <form v-else @submit.prevent="submit" class="flex flex-col gap-4">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input v-model="form.name" type="text" placeholder="Your name" class="bg-slate-900 border rounded-lg px-3 py-2.5 text-sm text-slate-100 placeholder-slate-600 outline-none transition-colors" :class="errors.name ? 'border-red-500' : 'border-slate-700 focus:border-[#f7600d]'" />
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input v-model="form.email" type="email" placeholder="you@example.com" class="bg-slate-900 border rounded-lg px-3 py-2.5 text-sm text-slate-100 placeholder-slate-600 outline-none transition-colors" :class="errors.email ? 'border-red-500' : 'border-slate-700 focus:border-[#f7600d]'" />
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input v-model="form.phone" type="tel" placeholder="+1 (555) 000-0000" class="bg-slate-900 border border-slate-700 focus:border-[#f7600d] rounded-lg px-3 py-2.5 text-sm text-slate-100 placeholder-slate-600 outline-none transition-colors" />
                    </div>
                    <div class="flex flex-col gap-2">
                        <label class="text-xs text-slate-500 font-semibold uppercase tracking-wider">I'm interested in</label>
                        <div class="flex gap-3">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input v-model="form.interest" type="radio" value="learn" class="accent-[#f7600d]" />
                                <span class="text-sm text-slate-300">Learning</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input v-model="form.interest" type="radio" value="buy" class="accent-[#f7600d]" />
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <textarea v-model="form.message" rows="3" placeholder="Tell us more..." class="bg-slate-900 border border-slate-700 focus:border-[#f7600d] rounded-lg px-3 py-2.5 text-sm text-slate-100 placeholder-slate-600 outline-none resize-none transition-colors" />
                    </div>
                    <button type="submit" :disabled="submitting" class="flex items-center justify-center gap-2 bg-[#f7600d] hover:bg-orange-600 disabled:opacity-50 text-white font-semibold px-5 py-2.5 rounded-lg text-sm transition-colors mt-1">
